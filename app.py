...
<style>
:root {
  --bg:#0f1115; --panel:#1b1f26; --panel2:#232832; --ink:#f3f4f6; --muted:#b6beca;
  --line:#3a414d; --line2:#515a68; --blue:#8ab4f8; --teal:#80cbc4;
  --amber:#f6c177; --red:#f28b82; --green:#a8dab5;
}
html, body, [data-testid="stAppViewContainer"], [data-testid="stMain"] {
  background:var(--bg)!important; color:var(--ink)!important;
}
.main .block-container { padding-top:.75rem; padding-bottom:2rem; max-width:1450px; background:var(--bg)!important; }
[data-testid="stHeader"], [data-testid="stToolbar"] { background:#0c0e12!important; }
[data-testid="stSidebar"] { background:#171b22!important; border-right:1px solid var(--line); }
[data-testid="stSidebar"] * { color:var(--ink)!important; }
[data-testid="stSidebar"] label { color:var(--muted)!important; }
[data-testid="stSidebar"] .stRadio div[role="radiogroup"] > label {
  height:42px; width:100%; box-sizing:border-box; background:#202631!important;
  border:1px solid var(--line)!important; border-radius:8px!important; padding:9px 12px!important;
  margin:7px 0!important; display:flex!important; align-items:center!important; box-shadow:none!important;
}
[data-testid="stSidebar"] .stRadio div[role="radiogroup"] > label:hover { background:#2a313d!important; border-color:#667085!important; }
[data-testid="stSidebar"] .stRadio div[role="radiogroup"] > label:has(input:checked) { background:#374151!important; border-color:var(--blue)!important; }
[data-testid="stSidebar"] .stRadio input { display:none!important; }
h1,h2,h3,h4,h5,h6,p,span,div,label { color:var(--ink)!important; }
.page-hero, .filter-panel, .section-card, .insight-box, .kpi, .definition-group {
  background:var(--panel)!important; border:1px solid var(--line); box-shadow:0 2px 8px rgba(0,0,0,.28);
}
.page-hero { border-left:5px solid var(--blue); padding:12px 16px; margin-bottom:12px; }
.page-hero h1 { margin:0 0 4px 0; font-size:25px; line-height:1.15; }
.page-hero p { margin:0; font-size:14px; color:var(--muted)!important; }
.filter-panel { padding:10px 12px 4px; margin-bottom:12px; }
.filter-panel input { background:#20242e!important; color:var(--ink)!important; border:1px solid var(--line2)!important; }
.kpi-grid { display:grid; grid-template-columns:repeat(5,minmax(145px,1fr)); gap:10px; margin:10px 0 14px; }
.kpi { min-height:88px; border-top:4px solid var(--accent); border-radius:8px; padding:10px 12px; }
.kpi-label { font-size:11px; color:var(--muted)!important; font-weight:800; text-transform:uppercase; letter-spacing:.035em; }
.kpi-num { font-size:28px; font-weight:900; line-height:1.05; margin:5px 0; }
.kpi-foot { font-size:12px; color:var(--muted)!important; line-height:1.25; }
.section-card { border-left:4px solid var(--accent,var(--blue)); border-radius:8px; padding:12px 14px; margin-bottom:12px; }
.section-card h3 { margin:0 0 10px 0; font-size:21px; }
.insight-row { display:grid; grid-template-columns:repeat(2,minmax(260px,1fr)); gap:10px; margin-bottom:12px; }
.insight-box { border-left:4px solid var(--accent,var(--blue)); border-radius:8px; padding:11px 12px; font-size:14px; }
.excel-table { width:100%; border-collapse:collapse; background:var(--panel)!important; font-size:13px; table-layout:fixed; }
.excel-table th { background:#303846!important; color:#f8fafc!important; border:1px solid var(--line2); padding:8px 9px; text-align:center; font-weight:850; }
.excel-table td { border:1px solid var(--line); padding:7px 9px; vertical-align:middle; background:#1f242d!important; color:var(--ink)!important; }
.excel-table tr:nth-child(even) td { background:#252b35!important; }
...
.bar-box:before { content:""; position:absolute; inset:0 auto 0 0; width:var(--w); background:linear-gradient(90deg,rgba(138,180,248,.65),rgba(138,180,248,.15)); }
.bar-box span { position:relative; z-index:1; font-weight:900; color:#fff!important; text-shadow:0 1px 2px #000; }
.definition-group { border-left:4px solid var(--teal); border-radius:8px; padding:12px 14px; margin-bottom:12px; }
.stDownloadButton button, .stButton button, .stFormSubmitButton button { background:#374151!important; color:#fff!important; border:1px solid var(--blue)!important; border-radius:7px!important; font-weight:800!important; }
[data-testid="stDataFrame"], [data-testid="stTable"] { background:var(--panel)!important; border:1px solid var(--line)!important; }
[data-testid="stAlert"] { background:#2b1d1f!important; border:1px solid #7f3a3a!important; }
...
def read_csv_or_excel(url: str) -> tuple[pd.DataFrame, str]:
...
def kpis(items: Iterable[tuple]) -> None:
...
def excel_bar_table(df: pd.DataFrame, label_col: str, value_col: str = "Records") -> None:
...
def chart(df: pd.DataFrame, label_col: str, value_col: str = "Records", title: str = ""):
    if df.empty or label_col not in df.columns or value_col not in df.columns:
        st.info(f"Chart cannot be rendered because required fields are missing: {label_col}, {value_col}.")
        return None
    data = df[[label_col, value_col]].dropna().head(20).copy()
    if data.empty:
        st.info("No chartable records available for this view.")
        return None
...
def downloads(df: pd.DataFrame, name: str, fig=None) -> None:
...
def source_page(title: str, df: pd.DataFrame, col: str, key: str) -> None:
    page_header(title)
    page = date_filter(df, key)
    t = count_table(page, col)
    st.markdown("<div class='section-card' style='--accent:#8ab4f8'><h3>Source table</h3>", unsafe_allow_html=True)
    excel_bar_table(t, col)
    downloads(t, key)
    st.markdown("</div>", unsafe_allow_html=True)
    st.markdown("<div class='section-card' style='--accent:#80cbc4'><h3>Chart view</h3>", unsafe_allow_html=True)
    fig = chart(t, col, title=title)
    downloads(t, f"{key}_chart_data", fig)
    st.markdown("</div>", unsafe_allow_html=True)
    if col == "Customer":
        for rows, cols, name in [("Customer", "Reason", "Customer × reason"), ("Customer", "Event type", "Customer × event type"), ("Customer", "Root Cause", "Customer × root cause")]:
            ...
            if not ct.empty:
                st.markdown(f"<div class='section-card' style='--accent:#a8dab5'><h3>{name}</h3>", unsafe_allow_html=True)
                st.dataframe(ct, use_container_width=True, hide_index=True)
                downloads(ct, name.lower().replace(" ", "_"))
                st.markdown("</div>", unsafe_allow_html=True)
...
def manual_entry_form(source_cols: list[str]) -> None:
    with st.expander("Add complaint / inquiry entry manually"):
        ...
        with st.form("manual_entry_form"):
            ...
            row = {
                "Email/JIRA Date": c1.date_input("Email/JIRA Date"),
...
if selected_page == "Executive Summary":
    ...
    kpis([
        ("Total records", len(page), f"{len(complaints)} complaints · {len(inquiries)} inquiries", "#8ab4f8"),
        ("Complaints %", f"{len(complaints)/total*100:.1f}%", "Share of selected records", "#f28b82"),
        ("Missed events", missed, f"{missed/total*100:.1f}% of selected records", "#f6c177"),
        ("People misses", people, "People-rooted records", "#b6beca"),
        ("Process/Product", f"{process}/{product}", "Process vs Product root causes", "#80cbc4"),
    ])
    top_customer = count_table(complaints, "Customer").head(1)
    top_reason = count_table(complaints, "Reason").head(1)
    st.markdown(f"<div class='insight-row'><div class='insight-box' style='--accent:#8ab4f8'><b>Most complaining customer:</b> {top_customer.iloc[0,0] if not top_customer.empty else 'No complaint customer in range'}.</div><div class='insight-box' style='--accent:#f6c177'><b>Leading complaint nature:</b> {top_reason.iloc[0,0] if not top_reason.empty else 'No complaint reason in range'}.</div></div>", unsafe_allow_html=True)
    kpis([
        ("High severity", int((page.get("Severity", pd.Series(dtype=str)).astype(str) == "High").sum()) if "Severity" in page.columns else 0, "Records needing leadership attention", "#f28b82"),
        ("RCA requested", int((page.get("RCA Requested", pd.Series(dtype=str)).astype(str) == "Yes").sum()) if "RCA Requested" in page.columns else 0, "Customer/Product/CS RCA asks", "#f6c177"),
        ("Fixed", int((page.get("Short Term Fix Status", pd.Series(dtype=str)).astype(str) == "Fixed").sum()) if "Short Term Fix Status" in page.columns else 0, "Completed short-term fixes", "#a8dab5"),
        ("RCA shared", int((page.get("Short Term Fix Status", pd.Series(dtype=str)).astype(str) == "RCA Shared").sum()) if "Short Term Fix Status" in page.columns else 0, "RCA shared or approved", "#80cbc4"),
        ("Clarified", int((page.get("Short Term Fix Status", pd.Series(dtype=str)).astype(str) == "Clarification Provided").sum()) if "Short Term Fix Status" in page.columns else 0, "Clarifications completed", "#8ab4f8"),
    ])
    st.markdown("<div class='section-card' style='--accent:#8ab4f8'><h3>Event Summary Intelligence</h3><p>This cockpit summarizes customer pain, missed-event patterns, complaint reasons, root causes, and recommended automation controls for the selected date range.</p></div>", unsafe_allow_html=True)
    for title, col in [("Top complaints by customer", "Customer"), ("Nature of complaints", "Reason"), ("Missed event types", "Event type"), ("Root cause split", "Root Cause"), ("Severity split", "Severity"), ("Automation opportunities", "Standard Automation Focus")]:
        if col in complaints.columns:
            ...
            st.markdown(f"<div class='section-card' style='--accent:#8ab4f8'><h3>{title}</h3>", unsafe_allow_html=True)
            excel_bar_table(t, col)
            fig = chart(t, col, title=title)
            downloads(t, title.lower().replace(" ", "_"), fig)
            st.markdown("</div>", unsafe_allow_html=True)
    ...
    if "Month Label" in page.columns and "Issue Type" in page.columns:
        ...
        downloads(monthly, "monthly_trend")
        st.markdown("</div>", unsafe_allow_html=True)
        y_cols = [c for c in ["Complaint", "Inquiry"] if c in monthly.columns]
        st.markdown("<div class='section-card' style='--accent:#80cbc4'><h3>Grouped monthly chart</h3>", unsafe_allow_html=True)
        ...
        fig.update_xaxes(tickfont=dict(color="#f3f4f6"), gridcolor="#303846")
        fig.update_yaxes(tickfont=dict(color="#f3f4f6"), gridcolor="#303846")
        st.plotly_chart(fig, use_container_width=True)
        downloads(monthly, "monthly_trend_chart_data", fig)
        st.markdown("</div>", unsafe_allow_html=True)
    else:
        st.info("Monthly trend requires Month/Reporting Month and Issue Type fields.")
elif selected_page == "SOURCE 02 · Fix status":
    source_page(selected_page, filtered, "Short Term Fix Status", "fix_status")
elif selected_page == "SOURCE 03 · Severity":
    source_page(selected_page, filtered, "Severity", "severity")
elif selected_page == "SOURCE 04 · Root cause":
    source_page(selected_page, filtered, "Root Cause", "root_cause")
elif selected_page == "SOURCE 05 · Top customers":
    source_page(selected_page, filtered, "Customer", "top_customers")
elif selected_page == "SOURCE 06 · Automation focus":
    source_page(selected_page, filtered, "Standard Automation Focus", "automation_focus")
elif selected_page == "DETAIL · Event workload":
    source_page(selected_page, filtered, "Event type", "event_workload")
elif selected_page == "Automation urgency":
    page_header(selected_page)
    page = date_filter(filtered, "urgency")
    complaints = page[page["Issue Type"].astype(str).eq("Complaint")] if "Issue Type" in page.columns else page
    t = urgency_table(complaints)
    st.markdown("<div class='section-card' style='--accent:#f6c177'><h3>Urgency table</h3>", unsafe_allow_html=True)
    st.dataframe(t, use_container_width=True, hide_index=True)
    downloads(t, "automation_urgency")
    st.markdown("</div>", unsafe_allow_html=True)
    if not t.empty:
        st.markdown("<div class='section-card' style='--accent:#80cbc4'><h3>Urgency score graph</h3>", unsafe_allow_html=True)
        fig = chart(t, "Standard Automation Focus", "Urgency Score", "Automation urgency score")
        downloads(t, "automation_urgency_chart_data", fig)
        st.markdown("</div>", unsafe_allow_html=True)
elif selected_page == "Dynamic Source Discovery":
    page_header(selected_page)
    page = date_filter(filtered, "discovery")
    disc = page[page["Standard Automation Focus"].astype(str).eq("Dynamic Source Discovery")] if "Standard Automation Focus" in page.columns else page.iloc[0:0]
    st.markdown("<div class='section-card' style='--accent:#80cbc4'><h3>Source-miss meaning</h3><p>Dynamic Source Discovery identifies event types, customers, reasons, feeds, keywords, or source coverage patterns that current sources are missing or under-detecting.</p></div>", unsafe_allow_html=True)
    ...
    for rows, cols, name in [("Customer", "Event type", "Customer × event type"), ("Event type", "Reason", "Event type × reason"), ("Customer", "Reason", "Customer × reason")]:
        ...
        if not ct.empty:
            ...
            downloads(ct, name.lower().replace(" ", "_"))
            st.markdown("</div>", unsafe_allow_html=True)
    st.markdown("<div class='section-card' style='--accent:#b6beca'><h3>Complete Dynamic Source Discovery records</h3>", unsafe_allow_html=True)
    st.dataframe(disc, use_container_width=True, hide_index=True, height=420)
    downloads(disc, "dynamic_source_discovery_complete")
    st.markdown("</div>", unsafe_allow_html=True)
